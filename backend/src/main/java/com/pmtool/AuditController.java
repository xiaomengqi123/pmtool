...
class AuditController {
    ...
    private Map<String,Object> view(OperationLog log){String name=log.userId==null?"系统":users.findById(log.userId).map(u->u.displayName).orElse("已删除用户");return Map.of("id",log.id,"userId",Optional.ofNullable(log.userId).orElse(0L),"userName",name,"action",log.action,"resourceType",log.resourceType,"resourceId",Optional.ofNullable(log.resourceId).orElse(0L),"detail",Optional.ofNullable(log.detail).orElse(""),"traceId",Optional.ofNullable(log.traceId).orElse(""),"createdAt",log.createdAt.toString());}
}
