...
class OrganizationController {
    ...
    @GetMapping("/roles") ApiResponse<?> roles(){service.requireAdmin();return ApiResponse.ok(roles.findAllByOrderByIdAsc().stream().map(r->Map.of("id",r.id,"code",r.code,"name",r.name,"description",Optional.ofNullable(r.description).orElse(""),"permissions",roles.findPermissionCodes(r.id))).toList());}
