...
class ProjectExtrasController {
    ...
    @PutMapping("/documents/{id}") ApiResponse<?> updateDocument(@PathVariable Long projectId,@PathVariable Long id,@RequestBody ProjectDocumentInput body){manager(projectId);ProjectDocument d=document(projectId,id);d.title=body.title();d.content=body.content();documents.save(d);service.log("UPDATE","PROJECT_DOCUMENT",id,d.title);return ApiResponse.ok(Map.of("id",d.id));}
    ...
    @PutMapping("/risks/{id}") ApiResponse<?> updateRisk(@PathVariable Long projectId,@PathVariable Long id,@RequestBody ProjectRiskInput body){manager(projectId);ProjectRisk r=risk(projectId,id);r.title=body.title();r.level=Optional.ofNullable(body.level()).orElse("medium");r.status=Optional.ofNullable(body.status()).orElse("open");r.ownerId=body.ownerId();risks.save(r);service.log("UPDATE","PROJECT_RISK",id,r.title);return ApiResponse.ok(Map.of("id",r.id));}
