...
class ProjectCollaborationController {
    ...
    private void apply(Milestone target,MilestoneInput input){target.name=input.name();target.dueDate=input.dueDate();target.status=Optional.ofNullable(input.status()).orElse("pending");}
