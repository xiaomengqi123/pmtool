...
public class PmToolController {
    ...
    private Map<String,Object> workLogView(WorkLog w){return Map.of("id",w.id,"taskId",w.taskId,"userId",w.userId,"hours",w.hours,"workDate",w.workDate,"description",Optional.ofNullable(w.description).orElse(""),"status",w.status,"reviewerId",Optional.ofNullable(w.reviewerId).orElse(0L),"reviewComment",Optional.ofNullable(w.reviewComment).orElse(""),"version",w.version==null?0:w.version);}
}
