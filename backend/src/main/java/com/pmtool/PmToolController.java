...
public class PmToolController {
    ...
    @DeleteMapping("/customers/{id}") ApiResponse<Void> removeCustomer(@PathVariable Long id){service.deleteCustomer(id);return ApiResponse.ok(null);}
    ...
    @DeleteMapping("/projects/{id}") ApiResponse<Void> removeProject(@PathVariable Long id){service.deleteProject(id);return ApiResponse.ok(null);}
    ...
    @PostMapping("/notifications/{id}/read") ApiResponse<Void> read(@PathVariable Long id){NotificationItem n=notifications.findById(id).orElseThrow(()->service.fail(40400,HttpStatus.NOT_FOUND,"通知不存在"));if(!Objects.equals(n.userId,service.current().id()))throw service.fail(40300,HttpStatus.FORBIDDEN,"无权限");if(!n.read){n.read=true;notifications.save(n);service.log("UPDATE","NOTIFICATION",n.id,"标记已读");}return ApiResponse.ok(null);}
    @PostMapping("/notifications/read-all") ApiResponse<Void> readAll(){Long userId=service.current().id();long changed=0;for(NotificationItem notification:notifications.findByUserIdOrderByCreatedAtDesc(userId))if(!notification.read){notification.read=true;notifications.save(notification);changed++;}if(changed>0)service.log("UPDATE","NOTIFICATION",null,"全部标记已读（"+changed+" 条）");return ApiResponse.ok(null);}
