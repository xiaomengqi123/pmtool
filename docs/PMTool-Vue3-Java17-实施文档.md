...
- 附件记录保存原始文件名、对象存储键、MIME 类型、文件大小、业务类型与业务 ID；文件本体不存入 MySQL。
...
- 服务端生成对象存储键，禁止直接使用用户提供的文件名或路径；上传接口预留病毒扫描/恶意文件检测的异步钩子。
- 下载前校验目标资源读取权限，统一使用短期签名 URL 或经后端转发；响应设置 `Content-Disposition: attachment`，避免浏览器直接执行活跃内容。
- 附件保存至 S3 协议兼容的对象存储。开发环境使用 MinIO；生产环境通过环境变量切换至任意兼容服务（例如阿里云 OSS 或腾讯云 COS）。对象存储访问凭据仅以环境变量注入。
...
| 3. 任务协作 | 任务、依赖、排序、对象存储附件、非实时通知 | 任务列表、看板拖拽、详情抽屉、附件与通知面板 | 拖拽更新状态/顺序，冲突可识别，附件单文件不超过 2 MB 且受权限保护。 |
...
- `application.yml` 只放非敏感默认项；数据库密码、JWT 密钥、对象存储凭据通过环境变量或受管配置注入。数据库名称为 `pmtool_db`。首个管理员的账号和初始密码也仅从环境变量读取。
...
| 附件 | S3 兼容对象存储；开发使用 MinIO、生产通过环境变量切换；文件类型不限；单文件不超过 2 MB。 |
