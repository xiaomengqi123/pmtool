...
| 检查项 | 结果 | 证据 |
| --- | --- | --- |
| 前端单元测试 | 通过 | `npm test`：Vitest 7/7 通过，覆盖工作台、项目、任务与工时角色入口、项目编辑版本传递，以及项目经理作为普通成员时的权限边界。 |
| 前端类型检查与构建 | 通过 | `npm run build`：`vue-tsc -b` 与 Vite 构建通过。 |
| 后端单元/上下文/安全测试 | 通过 | `mvn test`：28/28 通过，含用户目录授权与软删除、会话/通知/审计隔离、成员分配校验、认证到工时审批 HTTP 端到端闭环、项目乐观锁冲突、核心实体输入校验、任务/风险负责人范围、项目进度计算、附件响应头安全、客户资料与任务/工时项目范围、状态筛选分页、登录/JWT 与 OpenAPI 契约回归。 |
