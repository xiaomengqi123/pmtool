...
| 后端单元/上下文/安全测试 | 通过 | `mvn test`：15/15 通过，含用户角色、客户资料与任务/工时项目范围、状态筛选分页、任务软删除、状态枚举、登录/JWT 与 OpenAPI 契约回归。 |
...
| TC-LIST-01 | 客户、项目、任务支持关键字/状态筛选和分页 | 通过 | 服务端在数据范围授权后执行筛选与分页；三类列表均提供查询和分页控件。 |
