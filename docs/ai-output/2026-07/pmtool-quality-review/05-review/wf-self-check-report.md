...
| 后端单元/上下文/安全测试 | 通过 | `mvn test`：30/30 通过，含用户目录授权与软删除、会话/通知/审计隔离、成员分配校验、认证到工时审批 HTTP 端到端闭环、项目与排序乐观锁冲突、请求 traceId 与审计关联、核心实体输入校验、任务/风险负责人范围、项目进度计算、附件响应头安全、客户资料与任务/工时项目范围、状态筛选分页、登录/JWT 与 OpenAPI 契约回归。 |
